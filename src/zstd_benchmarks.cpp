...
int main(int argc, char **argv) {
    ...
    for(size_t i=0;i<count;i++) {
        ...
        uint64_t start,end;
        RDTSC_START(start);
        size_t csize = ZSTD_compress(out.data(), bound, input.data(), len, 1);
        RDTSC_FINAL(end);
        insert_cycles += end-start;
        (void)csize; // suppress unused warning
    }
    ...
    for(size_t i=0;i<count;i++) {
        ...
        RDTSC_START(start);
        size_t dsize = ZSTD_decompress(decompressed.data(), len, out.data(), out.size());
        RDTSC_FINAL(end);
