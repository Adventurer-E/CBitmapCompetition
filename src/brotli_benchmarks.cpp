...
int main(int argc, char **argv) {
    ...
    for(size_t i=0;i<count;i++) {
        ...
        size_t outsize = BrotliEncoderMaxCompressedSize(len);
        std::vector<uint8_t> out(outsize);
        uint64_t start,end;
        RDTSC_START(start);
        BROTLI_BOOL ok = BrotliEncoderCompress(BROTLI_DEFAULT_QUALITY, BROTLI_DEFAULT_WINDOW,
                                               BROTLI_MODE_GENERIC, len, input.data(), &outsize, out.data());
        RDTSC_FINAL(end);
    ...
    for(size_t i=0;i<count;i++) {
        ...
        if(!ok) { fprintf(stderr,"compression error\n"); return -1; }
        out.resize(outsize);
        totalsize += out.size();
        ...
        size_t decsize = len;
        RDTSC_START(start);
        BrotliDecoderResult r = BrotliDecoderDecompress(out.size(), out.data(), &decsize, decompressed.data());
        RDTSC_FINAL(end);
        iter_cycles += end-start;
        if(r != BROTLI_DECODER_RESULT_SUCCESS || decsize != len) { fprintf(stderr,"decoding error\n"); return -1; }
